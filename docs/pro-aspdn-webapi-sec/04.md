...
![image](images/sq.jpg) **注意**您已经在前面的章节中看到了 cookies，并且刚刚看到了 HTTPS。可以在创建 cookie 时指定它必须只通过 HTTPS 传输。通过将 *CookieHeaderValue* 对象的布尔*安全*属性设置为 true，您可以创建一个安全的 cookie。

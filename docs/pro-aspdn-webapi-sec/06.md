...
![image](images/sq.jpg) **注意**通过`ToXmlString()`方法生成的密钥是纯文本 XML 格式的，但是出于安全原因，您不应该将它们存储在文件系统中。必须改用密钥容器。`System.Security.Cryptography.CspParameters`可以帮助你使用密钥容器来存储密钥。

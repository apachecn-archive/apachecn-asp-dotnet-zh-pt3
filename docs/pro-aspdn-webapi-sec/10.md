...
3.  Issuer 是令牌颁发机构。如果令牌来自 RP 应用不信任的发行者，那么令牌及其包含的声明就没有面值。如果发行者被认为是可信的，那么代币就有价值，就像纸币一样。一张纸币可能只是一张皱巴巴、脏兮兮的纸，但它有能力购买东西，因为它是由人们信任的人发行的。这里需要特别注意的是，只有当 SWT 是由它信任的发行者发行并且令牌是专门为它发行的，RP 才必须兑现它。对于第一个检查，发布者字段很重要，而对于第二个检查，受众字段很重要。

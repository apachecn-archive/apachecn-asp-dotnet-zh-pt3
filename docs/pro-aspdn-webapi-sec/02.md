...
我们构建了我们的第一个 web API，并使用 JQuery AJAX 从 ASP.NET MVC 应用中使用该 API。我们试图使用流行的表单认证来保护我们的 web API。从这一尝试中得出的结论是，在其他 ASP.NET 应用中流行的技术——Web 表单和 MVC——尤其是表单认证，可能不适合 ASP.NET Web API。
...
本书中讨论的概念是独立于框架和语言的。然而，为了统一起见，您将使用 Visual Studio 2015、ASP.NET 5、MVC 6、Entity Framework 7 和 C#(任何受支持的 Windows 操作系统都可以)来开发本书中介绍的示例。如果需要，您可以轻松地将大多数示例移植到 MVC 5.x 甚至 Web 表单应用中。使用 ASP.NET 5，您可以构建面向。网芯还是。NET 框架。本书中介绍的许多例子都可以在这两个目标上运行，有些例子只能在。NET 框架。

...
学习设计模式包括理解它们的意图和使用指南，以及使用它们的实际经验。本书讨论了SOLID原则、四人组设计模式、企业应用架构的选择模式，并选择了 JavaScript 模式，试图让您很好地理解主题。它还提供了许多真实世界的例子，以使所讨论的主题更加相关和实用。有了这些知识，您就可以在应用中充分利用这些原则和模式的威力和灵活性。
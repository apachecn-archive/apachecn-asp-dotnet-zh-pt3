...
> **模型-视图-控制器(MVC)* *架构模式将应用分成三个主要组件:模型、视图和控制器。ASP.NET MVC 框架为创建 Web 应用提供了 ASP.NET Web 窗体模式的替代方案。ASP.NET MVC 框架是一个轻量级的、高度可测试的表示框架，它(与基于 Web 表单的应用一样)与现有的 ASP.NET 功能(如母版页和基于成员身份的验证)相集成。系统中定义了 MVC 框架。Web.Mvc 程序集。**

...
本节的目的只是展示如何在 ASP.NET 核心应用中集成 GraphQL。要了解更多关于 GraphQL 的信息，请访问 [`http://graphql.org`](http://graphql.org) 。
...
我们从探索 HATEOAS 的概念开始，包括它与 REST 的关系以及它如何帮助我们实现无状态 API。之后，我们深入研究了版本控制，并探索了一些在我们的 ASP.NET 核心应用中实现版本控制的方法。我们讨论了在应用中使用 Swagger，以及它如何为我们的 API 自动生成交互式文档。在我看来，我们以一记重击结束了这一章，首先实现了一个 GraphQL 端点，它将允许客户精确地查询他们需要我们的服务做什么。

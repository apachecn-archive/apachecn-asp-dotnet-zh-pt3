...
在前一章中，我们对 ASP.NET 核心和相关技术进行了简要概述。本章将着重于帮助你使用我们之前介绍的一些工具创建一个简单的 API 应用。

本章结束时，您将能够使用 ASP.NET 核心中的一个预定义模板创建一个新的 API 应用。我们将看看如何处理跨代码库的依赖性，同时仍然保持组件之间的松散耦合。本章还将介绍应用启动机制的自主性，以了解如何配置应用初始化过程，最后，我们将创建一个可供使用的简单 API 端点。
...
在 Web 部分，我们注意到两种类型的 ASP.NET Web 应用，一种是基于 ASP.NET 核心的，另一种是基于完整的。NET 框架。我们需要选择 ASP.NET 核心 Web 应用类型，并为我们的项目命名。
...
我们将从零开始，这样我们就可以了解 ASP.NET 核心 2.0 应用的各个重要方面。选择空模板，然后单击确定。
...
在整个 ASP.NET 核心中，我们将注意到依赖项和配置符合某种添加/使用风格，首先定义需要什么，然后定义如何使用它。通过明确指定我们需要的组件，它优化了性能，从而提高了应用的性能，因为我们只为我们使用的东西付费，而不是全部。
...
现在，我们已经很好地理解了 ASP.NET 核心如何初始化 web 主机和启动应用，让我们开始构建两个消费端点。
...
在这一章中，我们讲述了很多关于在 ASP.NET 核心中创建你的第一个应用的内容。在探索了 Visual Studio 2017 提供的新模板体验之后，我们直接进入了新项目系统的内部工作以及如何创建 web 主机。此外，我们对依赖注入进行了简要概述，并了解了在我们的应用中应用控制反转的重要性和好处。我们还剖析了应用启动过程，以完全理解它是如何工作的，最后，我们学习了如何使用应用构建器上提供的一些扩展方法来实现不同的 API 端点。

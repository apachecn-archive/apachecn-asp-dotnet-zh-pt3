...
Docker Hub 包含各种预打包应用的映像，但是对于 ASP.NET Core开发，Docker 的真正强大之处在于能够为 MVC 应用创建自定义映像。
...
这个映像是由微软制作的，它包含。NET 核心运行时和 ASP.NET Core包，编译成本机代码，以改善应用的启动。此映像不包含。NET SDK，这意味着在映像中使用 MVC 应用之前，必须对其进行准备，如“为映像准备应用”一节中所示。

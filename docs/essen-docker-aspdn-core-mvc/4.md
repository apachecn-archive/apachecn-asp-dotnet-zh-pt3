...
使用 Docker 的构件是图像和容器。映像是应用的模板以及运行该应用所需的文件。容器是从映像创建的，用于隔离执行应用，以便一个应用不会干扰另一个应用。在这一章中，我将解释如何创建和使用映像和容器，并演示如何使用 Docker 来封装一个 ASP.NET Core MVC 应用，以便在 Linux 和 Windows 服务器上使用。表 [4-1](#Tab1) 将本章放在上下文中。
...
Docker Hub 包含各种预打包应用的图像，但是对于 ASP.NET Core开发，Docker 的真正强大之处在于能够为 MVC 应用创建自定义图像。
...
在这种情况下，基础映像被称为`microsoft/aspnetcore`，我已经指定应该使用标记为`1.1.1`的版本，它包含。网络核心和 ASP.NET Core版本 1.1.1。
...
这个图像是由微软制作的，它包含。NET 核心运行时和 ASP.NET Core包，编译成本机代码，以改善应用的启动。此图像不包含。NET SDK，这意味着在图像中使用 MVC 应用之前，必须对其进行准备，如“为图像准备应用”一节中所示。
...
当您将 ASP.NET Core应用容器化时，所有编译的类文件、NuGet 包、配置文件和 Razor 视图都被添加到映像中。`COPY`命令将文件或文件夹复制到容器中。
...
ASP.NET Core运行时每次收到 HTTP 请求时都会写出一条消息，而`docker logs`命令会显示这些消息，如下所示:
...
除了我对`Index.cshtml`文件所做的更改之外，您可以看到一些文件是由 ASP.NET Core运行时创建的。有些文件与调试有关，我会在第 8 章中向你展示如何在容器中进行调试。
...
用于 ASP.NET Core容器的基本 Linux 映像包括 Bash shell，您可以通过运行清单 [4-25](#Par149) 中所示的命令来启动它。
...
需要与容器交互的最常见任务是使用编辑器修改配置文件，这些文件直到应用至少启动一次后才创建。ASP.NET Core应用的 Linux 容器的基本映像不包含编辑器，但是您可以使用以下命令添加一个编辑器:
...
键入`Control+C`分离命令提示符并让容器运行。(写出来的消息表明`Control+C`将终止应用，但这是来自 ASP.NET Core服务器 Kestrel 的消息，它不接收击键。当使用 Docker 时，很容易将容器与运行在其中的应用混淆。)

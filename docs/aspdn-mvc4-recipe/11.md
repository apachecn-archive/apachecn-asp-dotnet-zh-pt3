...
![9781430247739_Fig11-01.jpg](images/9781430247739_Fig11-01.jpg)
...
1.  右键单击 web 应用程序项目中的`Models`文件夹，并选择 Add ![image](images/arrow.jpg) New Item。
...
1.  右键单击`Models`文件夹，并选择添加![image](images/arrow.jpg)类。将新的类文件命名为 **AjaxDataGridModels.cs** 。
...
1.  通过在解决方案资源管理器中右键单击`Models`文件夹并选择 Add ![image](images/arrow.jpg) Class 来创建新的类文件。
...
7.  Modify the contents of this action, as shown in [Listing 11-12](#list12).
    ...
    ![image](images/sq.jpg) **提示**使用 C#零合并操作符。这种技术提供了一种非常简洁的方法来检查一个`null`值，然后提供一个值。这种技术显示在[清单 11-10](#list10) 中。在这里，我们测试可空的`Page`是否有值，如果有，将一个`int`变量页赋值为 1。语法`int page= Page??1`相当于写`int page = Page.HasValue ? Page.Value : 1`。
...
1.  右键单击脚本节点，并选择添加![image](images/arrow.jpg)新文件夹。
2.  将新文件夹命名为 **Home** 。
3.  右键单击`Home`文件夹，选择添加![image](images/arrow.jpg)新项目。
...
![image](images/sq.jpg) **提示**当开始一个大量使用客户端脚本库(如 jQuery 和 Knockout.js)的新项目时，你应该经常检查以确保你有最新版本的 NuGet 包。为此，请在解决方案资源管理器中右键单击 web 应用程序项目的“引用”节点，然后选择“管理 NuGet 包”。在“管理 NuGet 包”窗口中，单击“更新”节点。任何过期的包都会在这里列出。您可以通过单击每个软件包的更新按钮来更新它们。
...
![9781430247739_Fig11-02.jpg](images/9781430247739_Fig11-02.jpg)
...
![9781430247739_Fig11-03.jpg](images/9781430247739_Fig11-03.jpg)
...
1.  右键单击`Models`文件夹，并选择添加![image](images/arrow.jpg)类。
...
1.  通过单击查看![image](images/arrow.jpg)其他窗口![image](images/arrow.jpg)程序包管理器控制台打开程序包管理器控制台。
...
![9781430247739_Fig11-04.jpg](images/9781430247739_Fig11-04.jpg)
...
1.  在解决方案资源管理器中右键单击脚本节点，并选择添加![image](images/arrow.jpg)新文件夹。
2.  将文件夹命名为 **Home** 。
3.  右键单击`Home`文件夹并选择添加![image](images/arrow.jpg) JavaScript 文件。
4.  将文件命名为 **Peeps.js** 。
...
![9781430247739_Fig11-05.jpg](images/9781430247739_Fig11-05.jpg)
...
![9781430247739_Fig11-06.jpg](images/9781430247739_Fig11-06.jpg)
...
1.  在解决方案资源管理器中右键单击`Content`节点，并选择 Create ![image](images/arrow.jpg) New Folder。
2.  将文件夹命名为 **Home** 。
3.  右键单击新文件夹并选择添加![image](images/arrow.jpg)样式表。
...
![image](images/sq.jpg) **注意**为数据的过滤和排序提供参数的更标准的方法是使用 OData。
...
在我们编写测试之前，我们需要修改 web 应用程序项目中的`global.asax`文件，以便在`WebApiConfig.cs`中定义的路由可以在我们的单元测试中使用，如下所示:
...
![image](images/sq.jpg) **注意**对于 Web API 路由，路由数据中没有指定动作。这是因为在 Web API 中，动作与 HTTP 动词相关联，而不是与路由信息相关联。
...
这个问题的解决方案是使用 HTTP 代理应用程序，比如 Fiddler。HTTP 代理是一种拦截浏览器和服务器之间发送的数据的软件。Fiddler 是免费的，被广泛使用。NET 开发人员。它具有广泛的功能集，不仅可以用于调试 web 应用程序，还可以用于调试 Windows 应用商店应用程序。

提琴手可以从`www.fiddler2.com`下载。

工作原理

一旦安装了 Fiddler，您只需运行它就可以使用它来调试您的 web 应用程序。默认情况下，Fiddler 会监听所有网络适配器的 HTTP 流量。使用任何打开的浏览器，您都可以看到您提出的每个请求。如果您运行的是 Windows 8，除了浏览器请求之外，您还可以看到 Windows 8 应用程序发出的所有请求，例如对您的 live tiles 的更新。
...
![9781430247739_Fig11-07.jpg](images/9781430247739_Fig11-07.jpg)
...
![9781430247739_Fig11-08.jpg](images/9781430247739_Fig11-08.jpg)
